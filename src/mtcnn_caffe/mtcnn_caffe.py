...
def drawBoxes(im, boxes, points, texts):
    # print(points)
    x1 = boxes[:, 0]
    y1 = boxes[:, 1]
    x2 = boxes[:, 2]
    ...
    for i in range(x1.shape[0]):
        ...
        cv2.putText(im, texts[i], (int(x1[i]), int(y1[i])), cv2.FONT_HERSHEY_COMPLEX, 0.45, color, 1)
        for k in range(5):
            cv2.circle(im, (points[i][k], points[i][k + 5]), 1, (55, 255, 155), 2)
...
def detect_face(img, minsize, PNet, RNet, ONet, threshold, fastresize, factor):
    ...
    # points = np.zeros([total_boxes.shape[0],10], dtype=np.int)
